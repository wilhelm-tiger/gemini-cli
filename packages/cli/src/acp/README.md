...
```bash
# General pattern
npm test -w @wilhelm-tiger/gemini-cli -- src/acp/<test-file-name>.ts

# Example
npm test -w @wilhelm-tiger/gemini-cli -- src/acp/acpRpcDispatcher.test.ts
```
...
```bash
source ~/.nvm/nvm.sh && nvm use default && npm test -w @wilhelm-tiger/gemini-cli -- src/acp/acpSession.test.ts
```
