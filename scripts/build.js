...
if (process.env.CI) {
  ...
  console.log('Building @wilhelm-tiger/gemini-cli-core...');
  execSync('npm run build -w @wilhelm-tiger/gemini-cli-core', {
    stdio: 'inherit',
    cwd: root,
  });
  ...
  const parallelWorkspaces = workspaceInfo
    .map((w) => w.name)
    .filter((name) => name !== '@wilhelm-tiger/gemini-cli-core');
