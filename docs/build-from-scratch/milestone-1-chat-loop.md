...
- **TypeScript:** In `packages/study/agent-ts-minimal`
- **Python:** In `packages/study/agent-py-minimal`
- **Go:** In `packages/study/agent-go-minimal`
- **Rust:** In `packages/study/agent-rs-minimal`
...
2.  Build and run:
    ```bash
    npm run start -w @wilhelm-tiger/agent-ts-minimal
    ```

To run from `packages/study/agent-ts-minimal`:
...
`packages/study/agent-py-minimal`.
...
Run the Go agent directly from `packages/study/agent-go-minimal` using the
following command:
...
Run the Rust agent from `packages/study/agent-rs-minimal` using Cargo:
